...
const SignUpPage = () => {
	...
	const handleSubmit = (e) => {
		...
		signup(formData)
			...
			.catch((error) => {
				let errorMsg = error.response?.data?.message || "A apărut o eroare";
				if (errorMsg.includes("Password must be at least 6 characters")) {
					errorMsg = "Parola trebuie să aibă cel puțin 6 caractere";
				}
	...
	return (
		<div className='flex flex-col justify-center py-12 sm:px-6 lg:px-8'>
			...
			>
				<h2 className='mt-6 text-center text-3xl font-extrabold text-emerald-400'>Creează-ți contul</h2>
			</motion.div>
				...
				<div className='bg-gray-800 py-8 px-4 shadow sm:rounded-lg sm:px-10'>
					...
					<p className='mt-8 text-center text-sm text-gray-400'>
						...
						<Link to='/login' className='font-medium text-emerald-400 hover:text-emerald-300'>
							Login aici <ArrowRight className='inline h-4 w-4' />
						</Link>
					</p>
				</div>
			</motion.div>
